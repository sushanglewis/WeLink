...
| F-011 | 设置适配 | P1 | 1) 个人设置弹窗（通用/通知/显示/安全），其中账号/用户名只读展示；2) 系统设置弹窗（通用/高级/账号），与 Mattermost 原生设置对齐并补充本地行为项；3) 字段默认值与生效策略；4) 明确「触发通知的关键词」与桌面通知/托盘红点的上下游关系 | F-004、F-010 | AC-011 |
...
- **BR-021 立即生效 vs 保存生效**：
  ...
  - 保存后生效：头像、全名、触发通知的关键词、修改密码。
- **BR-022 字段只读**：用户名、账号（企业邮箱）从企业身份体系/Mattermost 同步，EAIC 应用层只读展示，禁止用户修改；UI 中应使用静态文本或禁用态输入框，并标注「只读」。
...
- **BR-029 网络断开**：检测到网络断开后显示离线提示 banner，不强制退出；恢复后自动重连 WebSocket 并刷新当前 WebView。
- **BR-030 Token 过期**：检测到 401 或登录态失效时，显示「登录已过期，请重新登录」提示，随后自动跳转回引导程序。
