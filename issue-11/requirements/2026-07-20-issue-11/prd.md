...
- 员工每次使用需主动打开浏览器、输入/收藏服务器地址、登录，流程割裂。
...
- **降低使用门槛**：安装包预配置服务器地址，员工打开应用即进入登录流程。
...
|------|------|--------|------|----------|------------------|
| US-001 | 企业内部员工 | 安装并打开 EAIC 后无需手动输入 Mattermost 服务器地址 | 快速进入登录流程，减少使用门槛 | F-001 安装包预配置 | 启动后直接进入 EAIC 品牌引导程序；服务器 URL 输入入口被隐藏或预填充 |
...
| F-002 | 应用级统一登录 | P0 | 1) Splash 启动页；2) 组织 URL 确认/编辑页；3) 企业邮箱/密码输入；4) 调用 Mattermost 认证 API；5) token 加密存储 | F-001、Mattermost 认证接口 | AC-002 |
| F-003 | 记住我 | P0 | 1) 默认勾选记住我；2) 保存 refresh token；3) token 刷新策略；4) 手动退出时清除 token | F-002、OS credential store | AC-002、AC-003 |
| F-004 | 原生一级导航 | P0 | 1) 左侧边栏（聊天/通讯录/AI 表格）；2) 选中态与未读红点；3) 侧边栏折叠/展开；4) 底部用户区与状态菜单 | F-002 | AC-004 |
...
| F-011 | 设置适配 | P1 | 1) 个人设置弹窗（通用/通知/显示/安全）；2) 系统设置弹窗（通用/高级/账号）；3) 字段默认值与生效策略；4) 中国用户习惯分组命名 | F-004、F-010 | AC-011 |
| F-012 | 文件下载/打开 | P1 | 1) 拦截 WebView 下载；2) 保存到默认/指定目录；3) 调用系统默认应用打开 | F-005、OS shell API | AC-012 |
| F-013 | 深度链接 | P2 | 1) 注册 `eaic://` 协议；2) 解析并定位到指定聊天 | F-005、OS deep-link API | AC-013 |
...
- **BR-002 组织 URL**：`server_url` 由安装包预配置，用户可在引导程序中查看/修改；正常企业分发场景下用户无需输入。
...
- **BR-006 登录失败提示**：账号/密码错误、网络断开、组织 URL 不可达时，在对应输入框下方显示 inline 错误提示，不弹窗阻断。
...
- **BR-014 通知内容**：通知标题显示发送者名称 + 应用名（如「李四 - EAIC」），内容显示消息摘要。
...
- **BR-017 托盘菜单**：托盘右键菜单固定为「打开主窗口」「标记为已读」「设置」「关于」「退出」。
...
- **BR-021 立即生效 vs 保存生效**：
  - 立即生效：主题、语言、通知开关、声音、开机自启、最小化到托盘、下载路径。
  - 保存后生效：头像、全名、提及词、修改密码。
- **BR-022 字段只读**：用户名、邮箱从 Mattermost 同步，应用层只读展示。
- **BR-023 语言默认**：`zh-CN`；时区跟随系统。

### 5.5 品牌规则

- **BR-024 品牌隐藏**：窗口标题、应用图标、Splash、About 页、外链中不暴露 Mattermost 品牌。
- **BR-025 企业信息**：About 页展示版本号、版权、企业名称、IT 支持联系方式、第三方开源许可声明。
- **BR-026 主题模式**：支持浅色 / 深色 / 跟随系统；切换主题时同步到 WebView 内 B/S 页面。

### 5.6 异常处理规则

- **BR-027 网络断开**：检测到网络断开后显示离线提示 banner，不强制退出；恢复后自动重连 WebSocket 并刷新当前 WebView。
- **BR-028 Token 过期**：检测到 401 或登录态失效时，显示「登录已过期，请重新登录」提示，随后自动跳转回引导程序。
- **BR-029 通知权限被拒**：若用户拒绝系统通知权限，在设置中显示提示引导用户前往系统设置开启。
- **BR-030 崩溃恢复**：应用崩溃后提供可选自动重启（默认开启），重启后保留上次登录状态。
...
- **Given** 用户已完成 EAIC 安装  
- **When** 用户首次启动应用  
- **Then** 应用展示 EAIC 品牌 Splash，并直接进入引导程序；用户无需手动输入服务器地址。

### AC-002 应用级统一登录

- **Given** 用户处于引导程序且已输入正确的组织 URL  
...
- **When** 浏览各分类设置项  
- **Then** 分组、命名、默认值符合中国用户习惯；主题/语言/开关即时生效；头像/全名/密码修改后需保存。
