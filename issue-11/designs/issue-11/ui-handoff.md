...
| | 全名 | 从 Mattermost 同步 | 修改后需保存 | 必填，长度 1-64 字符 |
| | 用户名 | 从 Mattermost 同步 | 只读 | - |
| | 邮箱 | 从 Mattermost 同步 | 只读 | - |
| **通知** | 桌面通知 | 开启 | 立即生效 | 若系统权限被拒，显示引导开启 |
| | 通知声音 | 开启 | 立即生效 | - |
| | 邮件通知 | 跟随服务端 | 立即生效 | - |
| | 提及词 | 用户名 | 修改后需保存 | 多个关键词用空格分隔 |
| **显示** | 主题 | 跟随系统 | 立即生效 | 同步到 WebView |
| | 语言 | zh-CN | 立即生效 | 可能需要刷新 WebView |
| **安全** | 修改密码 | - | 需确认旧密码并保存 | 旧密码校验失败 inline 提示 |

### 11.3 系统设置项

| 分类 | 设置项 | 默认值 | 生效方式 | 校验/异常 |
|------|--------|--------|----------|-----------|
| **通用** | 开机自启动 | 关闭 | 立即生效 | 需 OS 权限 |
| | 最小化到托盘 | 开启 | 立即生效 | - |
| | 下载路径 | 系统下载文件夹 | 选择后立即生效 | 路径无效时提示 |
| | 语言 | zh-CN | 立即生效 | - |
| **高级** | 缓存清理 | - | 点击清理后立即生效 | 清理完成后 toast 提示 |
| | 硬件加速 | 开启 | 立即生效 | 重启后生效 |
| | 日志级别 | 信息 | 立即生效 | - |
| **账号** | 退出登录 | - | 点击后立即生效 | 二次确认弹窗 |
