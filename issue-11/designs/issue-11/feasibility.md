...
- 预配置服务器 URL + SSO 显著降低员工使用门槛。
- 隐藏 Mattermost 品牌，强化企业自有 IM 认知。

### 成本

- **开发成本**：中。Tauri 技术栈轻量，团队可复用现有 Web 前端能力；原生部分主要集中在一级导航、托盘、通知、登录。
...
| 风险 | 可能性 | 影响 | 缓解措施 |
|------|:------:|:----:|----------|
| WebView 与 B/S 页面通信复杂 | 中 | 中 | 定义统一 JS Bridge 协议；提供调试工具 |
| Mattermost Web 升级破坏嵌入 | 中 | 高 | 锁定服务端版本；建立回归测试 |
| macOS 签名/公证流程阻塞 | 中 | 中 | 提前申请 Apple Developer；CI 自动化 |
| SSO 回调在桌面端适配困难 | 中 | 高 | 与 IT 确认协议；使用 deep link 回调 |
| 团队 Rust 能力储备不足 | 低 | 中 | Tauri 前端为主，Rust 仅用于原生能力封装 |
