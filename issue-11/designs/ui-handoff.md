...
| 2026-07-23 | v1.0 | 定版：按 PM 最终决策移除 SSO、统一通知触发条件、同步组织 URL 与消息预览规则 | Agent / PM |
...
| 项 | 当前假设 | 状态 |
|----|----------|------|
| 默认窗口尺寸 | 1280 × 800（最小 1024 × 640） | 已确认 |
| 是否支持最大化 | 是 | 已确认 |
| 是否支持全屏 | 是 | 已确认 |
| 是否支持窗口缩放 | 是，但 WebView 内容区按 B/S 页面自身响应式处理 | 已确认 |
| 侧边栏宽度 | 展开 240px，折叠 72px | 已确认 |
| 标题栏 | 完全自定义标题栏，包含企业品牌标题和窗口控制按钮 | 已确认 |
...
首次安装后打开应用，用户进入引导程序。流程包含组织 URL、已同步的企业邮箱/密码输入。
...
| 组织 URL 输入框 | 预配置服务器地址，允许用户修改 | `https://lgdg.cc`（安装包内置） | 必填；需符合 HTTPS URL 格式；提交前需可连接 | URL 格式错误/无法连接时输入框下方显示 inline 错误 |
...
- WebView 内容区与左侧导航之间需要可见 1px 分割线。
- 切换导航项时，WebView 加载对应 B/S 页面（已确认：每次切换重新加载）。
...
| 项 | 当前假设 | 状态 |
|----|----------|------|
| 触发条件 | 应用在线、应用未处于前台聚焦、用户开启桌面通知、收到新消息时触发 | 已确认 |
| 通知标题 | 发送者名称 + 应用名 | 待确认 |
| 通知内容 | 显示发送者 + 消息摘要 | 已确认 |
| 通知图标 | 企业应用图标；企业未提供时使用 EAIC 默认图标 | 已确认 |
| 点击行为 | 唤出窗口，切换到「聊天」导航，并通过 JS Bridge 定位到对应会话/频道 | 已确认 |
| 免打扰 | 跟随 Mattermost 现有能力支持；作为在线状态之一或独立开关 | 待确认（若 Mattermost 支持则支持） |
| 消息预览开关 | 是否显示消息内容预览 | 默认开启，可在个人设置中关闭 |

### 10.2 托盘图标

- 应用启动后常驻托盘。
- 托盘图标显示未读红点（数字或红点）。
- 右键托盘菜单：打开主窗口、标记为已读、设置、关于、退出。其中「标记为已读」的行为与范围遵循 Mattermost 原生实现。
- 点击托盘图标：唤出/隐藏主窗口。
...
- 点击侧边栏底部用户头像 → 弹出菜单 → 选择「个人设置」或「系统设置」。
- 设置页面以弹窗形式呈现。
...
| | 语言 | zh-CN | 立即生效 | 可能需要刷新 WebView；与 Mattermost Web 端「Display > Language」对应 |
...
| 窗口标题 | 应用名称 | 已确认 |
| 启动图 / Splash | 仅展示企业 Logo + 应用名称 EAIC | 已确认 |
| 关于页 | 版本号 + 版权信息 + 企业信息 + 第三方许可声明 | 已确认 |
| 主品牌色 | 企业提供 HEX 色值；缺省采用 Data UI Design `#00AE68` | 待确认 |
| 主题模式 | 浅色 / 深色 / 跟随系统，需要独立深色模式 | 已确认 |
...
- **全局快捷键**：遵循 Mattermost 原生快捷键习惯，仅做品牌样式统一；平台差异如下：
  - `Ctrl/Cmd + ,`：打开设置。
