...
def check_codebase_not_yet_documented():
    idx = PROJECT_ROOT / "docs" / "knowledge" / "00-index.md"
    if idx.exists():
        fail("Codebase knowledge index already exists")
    pass_check("Codebase not yet documented")


def _get_state_variable(name: str) -> str:
    state = load_state()
    if state is None:
        return ""
...
def check_oss_research_complete(change_name: str = ""):
    if not change_name:
        change_name = _get_state_variable("change_name")
    if not change_name:
        fail("change_name is required for oss_research_complete check")
    doc = PROJECT_ROOT / "docs" / "research" / f"{change_name}-oss-options.md"
    require_nonempty_file(doc, "OSS research")
    pass_check(str(doc))
...
def check_openspec_artifact_complete(change_name: str, design_id: str = ""):
    ...
    if design_id:
        ...
        required_refs = [
            f"{prefix}designs/{design_id}/tdd-plan.md",
            f"{prefix}designs/{design_id}/prototype.pen",
            f"{prefix}designs/{design_id}/design-review.md",
        ]
...
def validate_tdd_plan_complete(design_id: str):
    ...
    required_refs = [
        ...
        f"{prefix}designs/{design_id}/design-review.md",
        f"{prefix}designs/{design_id}/fields.md",
        f"{prefix}designs/{design_id}/ui-spec.md",
        f"{prefix}designs/{design_id}/prototype.pen",
    ]
...
def check_codebase_knowledge_ready():
    idx = PROJECT_ROOT / "docs" / "knowledge" / "00-index.md"
    require_nonempty_file(idx, "Codebase knowledge index")
    pass_check(str(idx))
...
ENTRY_CHECKS = {
    ...
    "codebase_not_yet_documented": check_codebase_not_yet_documented,
}
...
EXIT_CHECKS = {
    ...
    "oss_research_complete": check_oss_research_complete,
    "codebase_knowledge_ready": check_codebase_knowledge_ready,
}
